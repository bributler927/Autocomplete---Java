...
public class BinarySearchAutocomplete implements Autocompletor {
	...
	/*
	If there are M terms that match the prefix, then the simple
	method of finding the M matches, copying them to a list,
	sorting them in reverse weight order, and then choosing the
	first k of them will run in the total of the times given below.
	 */

	public List<Term> topMatches(String prefix, int k) {
		//create dummy give function an term to work with
		...
		//in case no such words exist
		if (prefix == null) {
			throw new NullPointerException();
		}


		//establishing of first and last indices
		...
		//created this variable to shorten and set list size
		int numResults = Math.min(k, pq.size());

		LinkedList<Term> matches = new LinkedList<>();
		for (int i = 0; i < numResults; i++) {
			matches.addFirst(pq.remove());
		}
