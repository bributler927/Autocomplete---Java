...
public class Term implements Comparable<Term> {
	...
	public static class PrefixOrder implements Comparator<Term> {
		...
		public int compare(Term v, Term w) {
			...
			//created these variables to lessen the amount of typing to cal v.word and w.word
			String vword = v.myWord;
			String wword = w.myWord;
			/*
			If prefix is higher than the length
			of either of the words, than the length of the shorter
			word is used as the prefixsize
			 */

			//I created this variable to make the code easier for me to read
			int currentsize = myPrefixSize;

			if (vword.length() > currentsize){
				vword = vword.substring(0, currentsize);
			}

			if (wword.length() > currentsize) {
				wword = wword.substring(0, currentsize);
			}
